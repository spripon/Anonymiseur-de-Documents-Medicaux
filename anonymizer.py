...
def looks_like_date_token(text: str) -> bool:
    ...
    - NE demande plus que le token soit 'pur' : la date peut être au milieu
      d'un mot (ex: 'DU24/10/2025', '24/1O/2O25XYZ').
    """
    raw = text.strip()
    if not raw:
        return False
    ...
    # cherche une sous-chaîne de type JJ[sep]MM[sep]AAAA
    pattern = r"[0-3]?\d[./\-\s][0-1]?\d[./\-\s]\d{2,4}"
...
def detect_pii_boxes_from_ocr(ocr: dict) -> List[Tuple[int, int, int, int]]:
    ...
    for key, line in line_items:
        ...
        trigger_stems = ("redact", "rédact", "redige", "rédig", "valide", "validé", "signe", "signé")
        skip_after_trigger = {"par", "electroniquement", "électroniquement", ":"}

        for i, w in enumerate(words):
            low_full = normalize_word(w["text"]).lower()
            if any(stem in low_full for stem in trigger_stems):
                j = i + 1
